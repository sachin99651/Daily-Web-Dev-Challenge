...
This project is a web-based **Playing Cards Layout** created as part of an advanced CSS Flexbox alignment and structural positioning lab. It demonstrates clean multi-axis item positioning, independent flex alignments, and responsive wrappers to render realistic poker-style playing cards using pure HTML and CSS.
...
* **Independent Cross-Axis Alignment**: Utilizes distinct `align-self` rule values to precisely distribute structural rank indicators across separate corners without hardcoded positional offsets:
    * **Top-Left Rank**: Locks tightly to the upper layout rim via `align-self: flex-start;`.
    * **Center Symbol Cluster**: Anchors seamlessly in the geographic middle via `align-self: center;`.
    * **Bottom-Right Rank**: Drops completely to the lower layout edge via `align-self: flex-end;`.
...
* `<div class="middle">`: Core canvas box handling the specific multi-item layout configurations for central suit symbols.
...
* **Dimensional Shadows**: Applies soft dark blur drop-shadow overlays (`rgba(0, 0, 0, 0.15)`) below the card surfaces to add visual depth against the dark background.
...
To view the playing cards layout locally:
1. Make sure your HTML script and `styles.css` file are located in the exact same workspace directory.
2. Run the HTML document inside any modern web browser.

---

## ⚙️ Technical Details

* **Font Typography**: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif system defaults.
* **Card Proportions**: `150px` width by `220px` height.
* **Corner Radii**: Rounded profiles secured at `10px`.
* **Palette Coordinates**: 
    * Dark void backdrop: `#1a1a1a`
    * Clean paper face: `#ffffff`
    * Core index ink: `#333333`
    * Red suit hue: `#e74c3c`
